...
> 请在使用前了解以下情况：
>
> * **任务连续性问题**: 在一个任务（无论是正常完成还是手动停止）结束后，程序内部状态可能不会被完全重置，导致直接启动下一个新任务时无法正确执行。
>   * **推荐的解决方案**: 重启本程序。这是最简单且最可靠的方法，可以确保为新任务提供一个干净的运行环境。
>   * **备用解决方案**: 如果不想重启程序，可以尝试以下操作来重置任务状态：首先点击“启动任务”，然后立即反复点击“停止任务”按钮直至其恢复初始状态，之后再重新启动新任务。
>
> * **停止任务按钮的可靠性**: “停止任务”按钮的响应并非总是即时的，在某些情况下可能需要多次点击才能成功停止。
